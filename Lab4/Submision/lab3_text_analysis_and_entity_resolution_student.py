...
def countTokens(vendorRDD):
    ...
    """
    return sum(vendorRDD.map(lambda x: len(x[1])).collect())
...
def findBiggestRecord(vendorRDD):
    ...
    """
    return vendorRDD.map(lambda x: (x[0],x[1],len(x[1]))).takeOrdered(1, lambda x: -x[2])
...
def tfidf(tokens, idfs):
    ...
    """
    tfs = [(x,float(tokens.count(x))/len(tokens)*idfs[x]) for x in tokens]
    tfIdfDict = dict(tfs)
    return tfIdfDict
...
avgSimDups = sum(trueDupsRDD.map(lambda x: x[1]).map(lambda x:x[0]).collect())/float(trueDupsCount)

nonDupsRDD = (sims
              .subtractByKey(goldStandard).map(lambda x:x[1]))
avgSimNon = sum(nonDupsRDD.collect())/float(nonDupsRDD.count())
...
commonTokens = (amazonInvPairsRDD
                .join(googleInvPairsRDD).map(lambda x: swap(x)).reduceByKey(lambda x,y: x+y)
                .cache())
