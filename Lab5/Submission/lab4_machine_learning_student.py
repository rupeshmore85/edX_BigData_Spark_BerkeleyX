...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    """
    averageRat = sum(IDandRatingsTuple[1])/float(len(IDandRatingsTuple[1]))
    return (IDandRatingsTuple[0],(len(IDandRatingsTuple[1]),averageRat))
...
movieIDsWithAvgRatingsRDD = movieIDsWithRatingsRDD.map(lambda x: getCountsAndAverages((x[0],x[1])))
...
trainingAvgRating = trainingRDD.map(lambda x: (x[1],x[2])).values().sum()/trainingRDD.map(lambda x: (x[1],x[2])).values().count()
...
myRatedMovies = [
     (myUserID,260, 3),(myUserID,1, 5),(myUserID,150, 4),(myUserID,527, 4),(myUserID,531, 4),
     (myUserID,588,4),(myUserID,595, 4),(myUserID,720,4),(myUserID,745, 3),(myUserID,919, 5)
...
myUnratedMoviesRDD = (moviesRDD
                      .filter(lambda x: x[0] not in [y[1] for y in myRatedMovies])
